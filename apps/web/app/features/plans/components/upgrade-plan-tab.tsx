...
export const UpgradePlanTab = ({ tenantType }: UpgradePlanTabProps) => {
  ...
  const currentPlanOrder = getPlanOrder(currentPlanName, tenantType);
  const isCurrentlyFree = isFreePlan(currentPlanName, tenantType);
  const plansPath = getPlansPath(tenantType);
  ...
  const gridCols = plans.length === 3 ? 4 : 3;
    ...
    <Box className="space-y-6">
      ...
      <Box className="mb-4">
        ...
        <Typography variant="body2" color="text.secondary">
          Seu plano atual:{" "}
          <strong>{getPlanDisplayName(currentPlanName, tenantType)}</strong>
        </Typography>
      </Box>
      ...
      <Grid container spacing={3}>
        {plans.map((plan) => {
          ...
          const planConfig = getPlanConfig(plan.name, tenantType);
